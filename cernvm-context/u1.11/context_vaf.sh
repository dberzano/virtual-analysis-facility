...
_EoF_
  ...
  echo '[elastiq]' >> "$Cfg"
  env | grep ^VafConf_Elastiq_ | while read ElastiqLine ; do
    Key=$(echo "$ElastiqLine" | cut -d= -f1 | tr '[:upper:]' '[:lower:]')
    Key=${Key:16}
    Val=$(echo "$ElastiqLine" | cut -d= -f2-)
    echo "$Key = $Val" >> "$Cfg"
  done

  # Prefix to the vmstart/vmstop scripts
